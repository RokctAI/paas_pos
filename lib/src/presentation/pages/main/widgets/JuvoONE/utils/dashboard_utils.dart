...
int calculateROSystemEfficiency(
  Map<String, dynamic>? roSystem,
  ...
  return ((membraneEfficiency + megaCharEfficiency + softenerEfficiency) / 3)
      .round();
}
...
int? calculateVesselEfficiency(String? lastReplaced) {
  ...
  int monthsSinceReplacement =
      currentDate.difference(replacementDate).inDays ~/ 30;
...
Map<String, double> calculateEnergyStats(
  List<Map<String, dynamic>> energyPurchases,
) {
  ...
  var lastMonthPurchases = energyPurchases.where((purchase) {
    DateTime purchaseDate = DateTime.parse(purchase['date']);
    return purchaseDate.month == lastMonth &&
        purchaseDate.year == lastMonthYear;
  });

  double thisMonthKwh = thisMonthPurchases.fold(
    0,
    (sum, purchase) => sum + double.parse(purchase['kwh'].toString()),
  );
  double lastMonthKwh = lastMonthPurchases.fold(
    0,
    (sum, purchase) => sum + double.parse(purchase['kwh'].toString()),
  );
...
Map<String, double> calculateExpenses(List<Map<String, dynamic>> expenses) {
  ...
  String thisWeekStart = now
      .subtract(Duration(days: now.weekday - 1))
      .toIso8601String()
      .split('T')[0];
  String thisMonthStart = DateTime(
    now.year,
    now.month,
    1,
  ).toIso8601String().split('T')[0];
  String lastMonthStart = DateTime(
    now.year,
    now.month - 1,
    1,
  ).toIso8601String().split('T')[0];
  String lastMonthEnd = DateTime(
    now.year,
    now.month,
    0,
  ).toIso8601String().split('T')[0];
  ...
  return {
    ...
    'thisWeek': expenses
        .where((e) => e['date'].compareTo(thisWeekStart) >= 0)
        .fold(0, (sum, e) => sum + e['amount']),
    'thisMonth': expenses
        .where((e) => e['date'].compareTo(thisMonthStart) >= 0)
        .fold(0, (sum, e) => sum + e['amount']),
    'lastMonth': expenses
        .where(
          (e) =>
              e['date'].compareTo(lastMonthStart) >= 0 &&
              e['date'].compareTo(lastMonthEnd) <= 0,
        )
        .fold(0, (sum, e) => sum + e['amount']),
  };
}
...
List<Map<String, dynamic>> checkMaintenanceAlerts(Map<String, dynamic> data) {
  ...
  DateTime backwashDate = DateTime.parse(
    data['maintenance']['lastBackwash'],
  ).add(Duration(days: data['maintenance']['periods']['backwash']));
  ...
  DateTime membraneDate = DateTime.parse(
    data['roSystem']['membranes'][0]['lastReplaced'],
  ).add(Duration(days: data['maintenance']['periods']['membrane']));
  if (currentDate.isAfter(membraneDate)) {
    alerts.add({
      'type': 'Membrane',
      'message':
          '${data['roSystem']['membranes'].length} membrane(s) need replacement',
      'dueDate': membraneDate,
    });
  }

  // Mega Char and Softener media replacement check
  DateTime mediaReplacementDate = DateTime.parse(
    data['maintenance']['mediaLastReplaced'],
  ).add(Duration(days: data['maintenance']['periods']['mediaReplacement']));
  if (currentDate.isAfter(mediaReplacementDate)) {
    alerts.add({
      'type': 'Media Replacement',
      'message':
          'Mega Char (${data['roSystem']['megaCharVessels'].length} vessel(s)) and Softener media need replacement',
      'dueDate': mediaReplacementDate,
    });
  }

  // Pre-filter replacement check
  DateTime preFilterDate = DateTime.parse(
    data['maintenance']['preFilterLastReplaced'],
  ).add(Duration(days: data['maintenance']['periods']['preFilter']));
  ...
  DateTime postFilterDate = DateTime.parse(
    data['maintenance']['postFilterLastReplaced'],
  ).add(Duration(days: data['maintenance']['periods']['postFilter']));
